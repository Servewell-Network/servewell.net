 ...
 * 
 * This script:
 * 1. Saves the current code state
 * 2. Reverts changes to the specified file(s)
 * 3. Regenerates JSON with "before" state
 * 4. Captures alignment data
 * 5. Restores the current code
 * 6. Regenerates JSON with "after" state
 * 7. Compares alignment data
...
class BeforeAfterGenerator {
  ...
  checkGitStatus() {
    try {
      ...
      if (status.trim()) {
        this.log('Warning: Uncommitted changes detected. Stashing...');
        execSync('git stash');
        this.stashCreated = true;
      }
    } catch (e) {
      this.error(`Git status check failed: ${e.message}`);
    }
  }
  ...
  saveCurrentState() {
    ...
    for (const file of this.targetFiles) {
      ...
      this.backups[file] = fs.readFileSync(fullPath, 'utf8');
    }
    this.log(`Saved ${this.targetFiles.length} file(s)`);
  }
  ...
  revertChanges() {
    ...
    try {
      ...
      this.log('Successfully reverted to HEAD');
    } catch (e) {
      this.error(`Git checkout failed: ${e.message}`);
    }
  }
   ...
   */
  restoreCurrentState() {
    this.log('Restoring current state...');
    for (const file of this.targetFiles) {
      fs.writeFileSync(path.join(this.workDir, file), this.backups[file]);
    }
    this.log('Restored all files');
  }
  ...
  findChapterFile(verseName) {
    const match = verseName.match(/^([A-Z]+)(\d+):/);
    if (!match) return null;

    const bookCode = match[1];
    const chapter = match[2].padStart(3, '0');
