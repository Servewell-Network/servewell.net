...
function renderChapterPage(
  ...
  return [
    ...
    `<main class="chapter-page mode-traditional-only" data-book="${escapeHtml(chapter.DocOrBookAbbreviation)}" data-chapter="${chapter.ChapterNumber}">`,
    ...
    `<input type="checkbox" id="column-toggle-traditional" data-column-toggle="traditional" checked />`,
...
function renderChapterNavHeader(prevInfo: AdjacentChapterInfo, nextInfo?: AdjacentChapterInfo): string {
  ...
  const nextArrow = nextInfo
    ...
    : '';
  return [
    `<div class="chapter-nav-header mode-traditional-only">`,
...
function renderSharedWordPopoverScript(): string {
  ...
  return [
    ...
    '        var bookName = pathParts.length >= 3 ? decodeURIComponent(pathParts[2]).replace(/-/g, " ") : "";',
    '        var fragment = "#";',
    '        if (grammarCode) fragment += "grammar=" + encodeURIComponent(grammarCode) + "&";',
    '        if (bookName) fragment += "document=" + encodeURIComponent(bookName);',
...
function renderColumnVisibilityScript(): string {
  ...
  return [
    ...
    '  const prevHeader = document.querySelector(".chapter-nav-header");',
    ...
    '',
    '  function readState() {',
    '    const fallback = { literal: false, traditional: true };',
    '    try {',
    ...
    '    if (prevHeader instanceof HTMLElement) {',
    '      prevHeader.classList.toggle("mode-both", showLiteral && showTraditional);',
    '      prevHeader.classList.toggle("mode-literal-only", showLiteral && !showTraditional);',
    '      prevHeader.classList.toggle("mode-traditional-only", !showLiteral && showTraditional);',
    '      prevHeader.classList.toggle("mode-none", !showLiteral && !showTraditional);',
    '    }',
    ...
    '  const header = page.querySelector(".snippet-grid-header");',
    '  if (header instanceof HTMLElement) {',
    '    let lastY = window.scrollY;',
    '    let armY = window.scrollY;',
    '    let ticking = false;',
    '    const THRESH = 6;',
    '    const SHOW_ALWAYS_TOP = 96;',
    '    const ARM_DISTANCE = 120;',
    '    let hideShowEnabled = false;',
    '    function enableHideShow() {',
    '      if (hideShowEnabled) return;',
    '      hideShowEnabled = true;',
    '      header.classList.add("is-scroll-aware");',
    '      armY = window.scrollY;',
    '      lastY = window.scrollY;',
    '    }',
    '    window.addEventListener("wheel", enableHideShow, { passive: true, once: true });',
    '    window.addEventListener("touchmove", enableHideShow, { passive: true, once: true });',
    ...
    '          return;',
    '        }',
    '        if (Math.abs(y - armY) < ARM_DISTANCE) {',
    '          header.classList.remove("hidden");',
    '          lastY = y;',
    '          ticking = false;',
    '          return;',
    '        }',
    '        if (y <= SHOW_ALWAYS_TOP) {',
    '          header.classList.remove("hidden");',
    '          lastY = y;',
    '          ticking = false;',
    '          return;',
    ...
    '          if (dy > 0) header.classList.add("hidden");',
...
function getChapterPageCss(): string[] {
  return [
    ...
    '  background: var(--page-bg);',
    '}',
    '.snippet-grid-header.is-scroll-aware {',
    '  transition: transform 220ms ease;',
    '}',
    '.snippet-grid-header.hidden {',
    ...
    '}',
    '.chapter-nav-header.mode-literal-only .snippet-row,',
    '.chapter-nav-header.mode-traditional-only .snippet-row {',
    '  grid-template-columns: minmax(0, 1fr);',
    '}',
    '.chapter-page.mode-literal-only .traditional-pane {',
    '  display: none;',
    '}',
    '.chapter-nav-header.mode-literal-only .traditional-pane {',
    '  display: none;',
    '}',
    '.chapter-page.mode-traditional-only .literal-pane {',
    '  display: none;',
    '}',
    '.chapter-nav-header.mode-traditional-only .literal-pane {',
    '  display: none;',
    '}',
    '.chapter-page.mode-none .snippet-grid {',
    '  display: none;',
    '}',
    '.chapter-nav-header.mode-none .snippet-grid {',
    '  display: none;',
    '}',
    '.verse-num {',
