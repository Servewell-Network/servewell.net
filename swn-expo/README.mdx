...
- tested with js turned off -- worked in web (except icons)!
- bullets in mdx don't show up in android -- must be the bug they mentioned in expo-mdx
- bullets are tough because what do you do when you run out of horizontal space?
- if nest in horizontal scrollview, no more text wrapping
- I think what should work is a fixed width on bullet TEXT so it wraps inside horz sv 
