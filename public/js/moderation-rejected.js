(function () {
  ...
  function render(items) {
    ...
    if (!Array.isArray(items) || items.length === 0) {
      listEl.hidden = true;
      setStatus('No rejected moderation items found.');
      return;
    }

    setStatus(`${items.length} rejected item${items.length === 1 ? '' : 's'}.`);
    listEl.hidden = false;
  ...
  async function load() {
    try {
      const response = await fetch('/api/moderation/verse-commentary/rejected', {
        headers: { Accept: 'application/json' }
      });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || 'Could not load rejected moderation items');
      render(Array.isArray(data.items) ? data.items : []);
    } catch (error) {
      if (listEl) listEl.hidden = true;
      setStatus(error instanceof Error ? error.message : 'Could not load rejected moderation items');
    }
  }
